...
CREATE OR REPLACE MATERIALIZED VIEW gold_tweet_aggregations AS
...
SELECT
    ...
    -- positive mentions
    COUNT(*) FILTER (
        WHERE predicted_sentiment = 'positive'
    ...
    -- total (exclude neutral)
    COUNT(*) FILTER (
        WHERE predicted_sentiment IN ('positive', 'negative')
    ) AS total_mentions,

    -- time range
    MIN(timestamp) AS first_mention_time,
    MAX(timestamp) AS last_mention_time

FROM tweets_gold

-- remove null mentions
