...
gold_df = (
    spark.read
    .format("delta")
    .table("workspace.default.tweets_gold")
)
...
mlflow.set_experiment("/Shared/tweet_sentiment_experiment")

model_name = "workspace.default.tweet_sentiment_model"
model_version = 1
data_path = "workspace.default.tweets_gold"

silver_history = spark.sql("DESCRIBE HISTORY workspace.default.tweets_silver")
silver_delta_version = silver_history.select("version").first()["version"]
...
with mlflow.start_run(run_name="tweet_sentiment_model_performance"):
